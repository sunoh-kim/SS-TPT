...
class DIFGSM(Attack):
    ...
    def input_diversity(self, x):
        img_size = x.shape[-1]
        img_resize = int(round(img_size * self.resize_rate))

        if self.resize_rate < 1:
            img_size, img_resize = img_resize, img_size  

        if img_size >= img_resize:
            rnd = int(img_resize)  
        else:
            rnd = int(torch.randint(low=img_size, high=img_resize + 1,
                                    size=(1,), device=x.device).item())
        ...
        if h_rem > 0:
            pad_top = int(torch.randint(0, h_rem + 1, (1,), device=x.device).item())
        else:
            pad_top = 0
        pad_bottom = h_rem - pad_top

        if w_rem > 0:
            pad_left = int(torch.randint(0, w_rem + 1, (1,), device=x.device).item())
        else:
            pad_left = 0
        ...
        return padded if torch.rand(1, device=x.device) < self.diversity_prob else x
